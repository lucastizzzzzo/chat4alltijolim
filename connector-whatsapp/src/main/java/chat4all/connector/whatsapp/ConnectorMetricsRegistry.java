...
/**
 * Centralized metrics registry for WhatsApp Connector.
 * 
 * Metrics collected:
 * - messages_sent_total{channel="whatsapp", status="success|failed"} - Counter
 * - connector_api_duration_seconds{channel="whatsapp"} - Timer (histogram)
 * - circuit_breaker_state{channel="whatsapp"} - Gauge (0=closed, 1=open, 0.5=half_open)
 * - circuit_breaker_transitions_total{channel="whatsapp", from, to} - Counter
 * 
...
public class ConnectorMetricsRegistry {
    ...
    private ConnectorMetricsRegistry() {
        ...
        this.messagesSentSuccess = Counter.builder("messages_sent_total")
                .description("Total messages sent through WhatsApp connector")
        ...
        this.apiDuration = Timer.builder("connector_api_duration_seconds")
                .description("Duration of WhatsApp API calls")
                .tag("channel", "whatsapp")
                .register(prometheusRegistry);
