...
# test-whatsapp-connector.sh - Integration test for WhatsApp Connector
# 
# Purpose: Validate Phase 5 implementation (WhatsApp Connector Mock)
#
# Test flow:
# 1. Authenticate (get JWT token)
# 2. Send message to WhatsApp recipient (whatsapp:+5511999999999)
# 3. Wait for connector to process
# 4. Verify connector logs show delivery simulation
# 5. Verify message was saved in Cassandra
#
# Usage: ./scripts/test-whatsapp-connector.sh
...
TEST_CONV="conv-whatsapp-test"
WHATSAPP_RECIPIENT="whatsapp:+5511999999999"

echo ""
echo "════════════════════════════════════════════════════════════════"
echo "  Phase 5: WhatsApp Connector - Integration Test"
echo "════════════════════════════════════════════════════════════════"
echo ""
...
# ========================================
# Test 2: Send message to WhatsApp recipient
# ========================================
echo -e "${YELLOW}[Test 2]${NC} Sending message to WhatsApp recipient..."
...
MESSAGE_RESPONSE=$(curl -s -X POST "$API_URL/v1/messages" \
    ...
    -d "{
        ...
        \"content\": \"Hello from WhatsApp connector test!\"
    }")
...
# ========================================
echo -e "${YELLOW}[Test 3]${NC} Waiting for WhatsApp connector to process message..."
...
# ========================================
echo -e "${YELLOW}[Test 4]${NC} Checking WhatsApp connector logs..."

# Check if connector container is running
if ! docker ps --format '{{.Names}}' | grep -q "connector-whatsapp"; then
    echo -e "${RED}✗ WhatsApp connector container not running${NC}"
    echo "Please start with: docker-compose up -d connector-whatsapp"
    exit 1
fi

CONNECTOR_LOGS=$(docker logs chat4alltijolim_connector-whatsapp_1 2>&1 | tail -50)
...
if echo "$CONNECTOR_LOGS" | grep -q "Delivered to"; then
    echo -e "${GREEN}✓ Connector simulated WhatsApp API delivery${NC}"
else
...
if echo "$ROUTER_LOGS" | grep -q "Routing to whatsapp-outbound"; then
    echo -e "${GREEN}✓ Router correctly identified WhatsApp recipient${NC}"
    echo -e "${GREEN}✓ Message routed to whatsapp-outbound topic${NC}"
else
    echo -e "${YELLOW}⚠ Routing log not found (may have scrolled past)${NC}"
    echo "Recent router logs:"
    echo "$ROUTER_LOGS" | grep -i "routing\|whatsapp" || echo "No routing logs found"
fi
...
if echo "$KAFKA_TOPICS" | grep -q "whatsapp-outbound"; then
    echo -e "${GREEN}✓ whatsapp-outbound topic exists${NC}"
else
    echo -e "${RED}✗ whatsapp-outbound topic not found${NC}"
...
# ========================================
echo "════════════════════════════════════════════════════════════════"
echo -e "${GREEN}✓ WhatsApp Connector tests passed!${NC}"
echo "════════════════════════════════════════════════════════════════"
echo ""
echo "✅ Test Results:"
echo "  [1] ✓ JWT authentication"
echo "  [2] ✓ Message sent to WhatsApp recipient"
...
echo "   • Router-worker detects 'whatsapp:' prefix in sender_id"
echo "   • Routes message to whatsapp-outbound topic (not local delivery)"
